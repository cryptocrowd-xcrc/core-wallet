...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zxlrspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 XLR at the same time
...
enum ZerocoinSpendStatus {
    ...
    ZXLR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXLR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZXLROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzxlrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzXLRTracker> zxlrTracker;
    ...
    void setZWallet(CzXLRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zxlrTracker = std::unique_ptr<CzXLRTracker>(new CzXLRTracker(strWalletFile));
    }

    CzXLRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zXLR reset */
    boost::signals2::signal<void()> NotifyzXLRReset;
