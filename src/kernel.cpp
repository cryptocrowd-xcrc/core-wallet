...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZXLRStake(spend));
    } else {
        ...
        CXLRStake* xlrInput = new CXLRStake();
        xlrInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(xlrInput);
    }
