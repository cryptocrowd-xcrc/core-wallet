...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cryptocrowd balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cryptocrowd/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cryptocrowd/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zxcrcObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zxcrcObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zXLRsupply", zxcrcObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cryptocrowdaddress\"\n"
            "\nReturn information about the given cryptocrowd address.\n"
            "\nArguments:\n"
            "1. \"cryptocrowdaddress\"     (string, required) The cryptocrowd address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cryptocrowdaddress\", (string) The cryptocrowd address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: CRyptoCrowd address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cryptocrowd addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cryptocrowd address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cryptocrowdaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cryptocrowdaddress\"  (string, required) The cryptocrowd address to use for the signature.\n"
