...
public:
    ...
    bool WriteZXLRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXLRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXLRSeed_deprecated(uint256& seed);
    bool EraseZXLRSeed();
    bool EraseZXLRSeed_deprecated();

    bool WriteZXLRCount(const uint32_t& nCount);
    bool ReadZXLRCount(uint32_t& nCount);
