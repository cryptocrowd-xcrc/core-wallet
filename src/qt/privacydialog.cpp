                                                          ...
                                                          currentBalance(-1)
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzXLR->setToolTip(tr("zXCRC is currently disabled due to maintenance."));

        ui->pushButtonSpendzXLR->setEnabled(false);
        ui->pushButtonSpendzXLR->setToolTip(tr("zXCRC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzXLR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXCRC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
...
void PrivacyDialog::on_pushButtonSpendzXLR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXCRC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
{
    ZXCRCControlDialog* zXLRControl = new ZXCRCControlDialog(this);
    zXLRControl->setModel(walletModel);
    zXLRControl->exec();
}

void PrivacyDialog::setZXCRCControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzXLR()
{
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XCRC </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZXCRCControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXCRCControlDialog::GetSelectedMints();
    }
    ...
    ZXCRCControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " XCRC, ";
