...
void PrivacyDialog::on_pushButtonMintzXLR_clicked()
{
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zXLR, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzXLR_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zXLR, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
...
void PrivacyDialog::sendzXLR()
{
    ...
    if (!ZXLRControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZXLRControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zXLR was spent successfully update the addressbook with the label
    ...
    ZXLRControlDialog::setSelectedMints.clear();
    ui->labelzXLRSelected_int->setText(QString("0"));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zxlrTracker->GetMints(false);
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zXLR "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXLR </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXLR </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zXLR </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzXLR->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        // Mint zXLR
        ui->pushButtonMintzXLR->setEnabled(true);
        ui->pushButtonMintzXLR->setToolTip(tr("PrivacyDialog", "Enter an amount of XLR to convert to zXLR", 0));

        // Spend zXLR
        ui->pushButtonSpendzXLR->setEnabled(true);
        ui->pushButtonSpendzXLR->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
