...
std::set<std::string> ZXLRControlDialog::setSelectedMints;
std::set<CMintMeta> ZXLRControlDialog::setMints;

bool CZXLRControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZXLRControlDialog::COLUMN_DENOMINATION || column == ZXLRControlDialog::COLUMN_VERSION || column == ZXLRControlDialog::COLUMN_CONFIRMATIONS)
...
void ZXLRControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZXLRControlWidgetItem* itemDenom(new CZXLRControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZXLRControlWidgetItem *itemMint = new CZXLRControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zxlrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zxlrTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zXLR seed used to mint this zXLR is not the same as currently hold in the wallet";
            else
...
void ZXLRControlDialog::updateLabels()
{
    ...
    privacyDialog->setZXLRControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXLRControlDialog::GetSelectedMints()
{
