...
std::list<std::string> ZXCRCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXCRCControlDialog::listMints;

ZXCRCControlDialog::ZXCRCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXCRCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXCRCControlDialog::~ZXCRCControlDialog()
{
    delete ui;
}

void ZXCRCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXCRCControlDialog::updateList()
{
...
void ZXCRCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXCRCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXCRC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXCRCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXCRCControlDialog::GetSelectedMints()
{
...
void ZXCRCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
