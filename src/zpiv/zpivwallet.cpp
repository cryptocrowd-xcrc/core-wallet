...
CzXLRWallet::CzXLRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxlr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXLRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXLRSeed_deprecated()) {
                    LogPrintf("%s: Updated zXLR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxlr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxlr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXLRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXLRCount(nCountLastUsed);
    else if (!walletdb.ReadZXLRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXLRWallet::Lock()
{
    seedMaster = 0;
}

void CzXLRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXLRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXLR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXLRWallet::LoadMintPoolFromDB()
{
...
void CzXLRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXLRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXLRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxlrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXLRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXLR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxlrTracker which also adds to database
    pwalletMain->zxlrTracker->Add(dMint, true);
...
void CzXLRWallet::SeedToZXLR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXLRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXLRWallet::GenerateDeterministicZXLR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXLRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXLRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXLRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
