// Copyright (c) 2018 The Solaris developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zxlr/deterministicmint.h>
#include "zxlrtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzXLRTracker::CzXLRTracker(std::string strWalletFile)
{
...
CzXLRTracker::~CzXLRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXLRTracker::Init()
{
...
bool CzXLRTracker::Archive(CMintMeta& meta)
{
...
bool CzXLRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXLRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXLRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXLRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzXLRTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzXLRTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzXLRTracker::GetSerialHashes()
{
...
CAmount CzXLRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXLRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXLRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXLRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXLRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXLRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXLRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXLRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXLRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXLRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzXLRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzXLRWallet* zXLRWallet)
{
    bool iszXLRWalletInitialized = (NULL != zXLRWallet);
    CMintMeta meta;
    ...
    if (! iszXLRWalletInitialized)
        zXLRWallet = new CzXLRWallet(strWalletFile);
    meta.isSeedCorrect = zXLRWallet->CheckSeed(dMint);
    if (! iszXLRWalletInitialized)
        delete zXLRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzXLRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXLRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXLRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXLRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXLRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXLRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzXLRWallet* zXLRWallet = new CzXLRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zXLRWallet);
        }
        delete zXLRWallet;
        LogPrint("zero", "%s: added %d dzxlr from DB\n", __func__, listDeterministicDB.size());
    }
