...
- [Building CRyptoCrowd](#building-solaris)
...
Clone the git repositories for solaris and gitian and then checkout the solaris version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/solaris-crypto/solaris.git
cd solaris
...
Follow the instructions in [doc/release-process.md](release-process.md) in the solaris repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the solaris repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/solaris/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/solaris-crypto/solaris
...
For example:
```bash
URL=https://github.com/crowning-/solaris.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit solaris=${COMMIT} --url solaris=${URL} ../solaris/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit solaris=${COMMIT} --url solaris=${URL} ../solaris/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit solaris=${COMMIT} --url solaris=${URL} ../solaris/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/solaris-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/solaris-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/solaris-build.assert
```
...
[solaris/gitian.sigs](https://github.com/solaris-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
